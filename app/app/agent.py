...
# Define peer agents for demonstration
peer_agent_1 = Agent(
    name="PeerAgent1",
    model="gemini-2.5-flash",
    instruction="You are PeerAgent1. Handle cost sharing and payment queries.",
    tools=[cost_agent, booking_agent],
)

peer_agent_2 = Agent(
    name="PeerAgent2",
    model="gemini-2.5-flash",
    instruction="You are PeerAgent2. Handle translation and feedback queries.",
    tools=[translation_agent, feedback_agent],
)

# Sub-agents for trip planning
sub_agent_itinerary = Agent(
    name="ItinerarySubAgent",
    model="gemini-2.5-flash",
    instruction="You are ItinerarySubAgent. Generate and adjust itineraries.",
    tools=[itinerary_agent_workflow, realtime_adjustment_agent],
)

# Parent agent (root_agent) orchestrates conversation and transfers control
root_agent = SequentialAgent(
    ...
    instruction=(
        "You are the RootAgent. Begin every conversation. "
        "You may transfer control to sub-agents by name: 'ItinerarySubAgent', 'PeerAgent1', 'PeerAgent2'. "
        "Decide which agent should handle each user query based on their description. "
        "If a query is about itinerary, use 'ItinerarySubAgent'. For cost/payment, use 'PeerAgent1'. For translation/feedback, use 'PeerAgent2'. "
        "Always return control to yourself after sub-agent completes its task."
    ),
)
